...
gulp.task('init', async () => {
  ...
  web3 = new Web3(wsp, null, { transactionConfirmationBlocks: 1 });
...
gulp.task('deploy-twins', ['init'], async () => {
  await deployTwins(runtimes, runtimeConfig, runtimeConfig.deployTwins);
});



gulp.task('default', ['deploy-twins']);
