...
export async function startDApp(container: any, dbcpName: any, dappEnsOrContract: any, dappBaseUrl: any) {
  ...
  const vueCoreDbcp = await dappBrowser.System.import(`bmvi.${ domainName }!ens`);
  const bmviBaseUrl = dappBrowser.dapp.getDAppBaseUrl(vueCoreDbcp,
    `${ vueCoreDbcp.name }.${ domainName }`);
